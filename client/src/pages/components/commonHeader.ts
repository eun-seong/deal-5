...
export default function (attrs: HTMLElementAttrs, title: string) {
  const _leftArrow = createElem('button', { class: 'header-left-arrow svg-icon' } as HTMLElementAttrs);
  _leftArrow.innerHTML = svgIcons.chevronLeft;

  const _headerTitle = createElem('div', { class: 'header-title' } as HTMLElementAttrs);
